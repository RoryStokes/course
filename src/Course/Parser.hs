...
mapParser ::
  ...
  -> Parser a
  -> Parser b
mapParser f (P g) = P (\i -> case g i of 
  Result ii a -> Result ii (f a)
...
bindParser ::
  ...
  -> Parser a
  -> Parser b
bindParser f (P g) = P (\i -> case g i of
  Result r x -> parse (f x) r
  ErrorResult e -> ErrorResult e)
...
--  Nil -> ErrorResult UnexpectedEof
--  h :. t -> if p h then Result t h else ErrorResult Failed)
...
natural ::
  Parser Int
natural = flbindParser (list1 digit) (\x -> case read x of 
    Empty -> failed
    (Full y) -> valueParser y)

-- bindParser :: (a -> Parser b) -> Parser a -> Parser b

--
-- | Return a parser that pr-oduces a space character but fails if
--
--   * The input is empty.
--
...
--sequenceParser = foldRight (\p r -> 
 -- bindParser (\x -> mapParser ((:.) x) r) p) (valueParser Nil)
...
firstNameParser = 
  do
    ...
    pure (h :. t)
  --flip (:.) <$> list lower <*> upper
-- firstNameParser = bindParser (\h -> mapParser(\t -> h :. t) (list lower)) upper
...
--  flbindParser (spaces1 >>> firstNameParser) (\b ->
--  flbindParser (spaces1 >>> surnameParser) (\c -> 
--  flbindParser (spaces1 >>> smokerParser) (\d ->
--   flmapParser (spaces1 >>> phoneParser) (\e ->
--    Person a b c d e)))))
