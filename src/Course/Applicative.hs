...
instance Applicative Optional where
  ...
  (<*>) ::
    ...
    -> Optional a
    -> Optional b
  (<*>) fs as = bindOptional (\f -> mapOptional f as) fs
...
filtering fp (h :. t) = (\p r -> if p then h :. r else r) <$> (fp h) <*> (filtering fp t)


-- foldRight (\a r -> (\p -> if p then (a :. r) else r) <*> (f a)) (pure Nil) list
-----------------------
-- SUPPORT LIBRARIES --
-----------------------
